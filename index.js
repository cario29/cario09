...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: 'Espera, ¿te casarás? ',
					success: '️❬ ✔ ❭ Pronto vey 🖤',
					levelon: '❬ ✔ ❭ *leveling * * habilitado * ',
					leveloff: ' ❬ X ❭  *leveling* *desativado*',
					levelnoton: '❬ X ❭ *leveling no ativado*',
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*Bueno, falla, vuelve a intentarlo ^ _ ^ * ',
				Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD Cariomaster*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Cariosito? Este é um recurso especial para o Cariosito❌',
					ownerB: '[❗] Cariosito? Este é um recurso especial para o Cariosito❌',
			...
			const ownerNumber = ["595976971273@s.whatsapp.net","558981246187@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"595976971273@s.whatsapp.net","558981246187@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["595976971273@s.whatsapp.net","558981246187@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["595976971273@s.whatsapp.net","558981246187@s.whatsapp.net"]//ubah nomor lo
			const premium = ["595976971273@s.whatsapp.net","558981246187@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
        ...
        if (messagesC.includes("negro de mierda")){
		if (!isGroup) return
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("porque sos negro")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('cara, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
